...
{
    public class AttackIfSurroundedAction: AbstractAction
    {
        ...
        {
            isMet = context?.SourceEntity is Player &&
                    context.PlayingState != null &&
                    context.PlayingState.GetAdjacentEnemies(context.SourceEntity.positionRowCol).Count >= 2;
            return true;
        }
        ...
        {
            return Amount + " <attack> If you have not moved this turn.";
        }

        public override string GetText(CardActionPreview preview)
        {
            int finalAmount = preview.GetFirstFinalValue(CardPreviewKeys.Damage, Amount);
            return preview.FormatValue("<attack>", Amount, finalAmount) + " if you have not moved this turn.";
        }
